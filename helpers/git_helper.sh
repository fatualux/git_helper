...
# DEPENDENCIES: bash git zenity github-cli gitlab-cli xclip
# USAGE: ./git_helper.sh
# AUTHOR: FZ
...
RepoCreator() {
  ...
  RepoTypeChooser
  RepoNameChooser
  echo "https://www.${PLATFORM}.com/$GIT_USER/$REPO_NAME" | xclip -selection clipboard
  echo "New repository address copied to clipboard." && sleep 1
  $CLI repo create "$REPO_NAME" $O_TYPE
  echo "$TYPE repository created: $REPO_NAME"
}

RepoRemover() {
  PlatformChooser
  RepoNameChooser
  $CLI repo delete "$REPO_NAME"
...
Contribute() {
  ...
  git remote add origin $OFFICIAL_REPO_NAMESITORY
